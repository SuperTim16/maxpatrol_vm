...
class MaxPatrolApp(QMainWindow):
    def __init__(self):
        ...
        self.output_button.clicked.connect(self.connect_ssh)
    ...
    def connect_ssh(self):
        ...
        ssh_connect(host, port, username, password, db_params)
...
def ssh_connect(host, port, username, password, db_params):
    try:
        client = paramiko.SSHClient()
        ...
        execute_and_write_to_postgres(client, host, "uname -a", "os_info", db_params)
        execute_and_write_to_postgres(client, host, f"nmap -p- {host} | grep 'open'", "ports", db_params)
...
def execute_and_write_to_postgres(client, host, command, column, db_params):
    try:
        stdin, stdout, stderr = client.exec_command(command)
        command_output = stdout.read().decode()
        logging.info(f"Выполнена команда '{command}'")

        write_to_postgres(host, command, command_output, column, db_params)

        log_result(True, "Operation completed successfully.")
    except Exception as e:
        log_result(False, f"Operation failed. Error: {str(e)}")

def write_to_postgres(host, command, output, column, db_params):
    try:
